...
const _geometrySchema = mongoose.Schema({
   type: {
      type: {
         geomCheckFn: function(typeStr) {
            if (typeStr === 'Polygon') return polygonSchema
            else if (typeStr === 'MultiPolygon') return multiPolygonSchema
         }
      }
   }
})
